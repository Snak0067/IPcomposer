...
class IpComposerModel(nn.Module):
    ...
    def get_image_embeds(self, device, pil_image=None, clip_image_embeds=None, num_images_per_prompt=1, do_classifier_free_guidance=False):
        # 图像嵌入列表
        image_embeds = []

        # 如果提供了 PIL 图像，则进行图像编码
        if pil_image is not None:
            ...
            # 将预处理后的图像输入到 `ip_image_encoder` 中获取图像嵌入
            clip_image_embeds = self.ip_image_encoder(clip_image.to(device, dtype=torch.float16)).image_embeds
        else:
            # 若直接提供 `clip_image_embeds`，确保其数据类型和设备正确
            clip_image_embeds = clip_image_embeds.to(device, dtype=torch.float16)

        # 使用 `image_proj_model` 对图像嵌入进行投影
        image_prompt_embeds = self.image_proj_model(clip_image_embeds)
        
        # 处理 `do_classifier_free_guidance` 模式的负向引导嵌入
        if do_classifier_free_guidance:
            uncond_image_prompt_embeds = self.image_proj_model(torch.zeros_like(clip_image_embeds))
        else:
            uncond_image_prompt_embeds = None

        # 重复嵌入以匹配 `num_images_per_prompt` 数量
        image_prompt_embeds = torch.cat([image_prompt_embeds] * num_images_per_prompt, dim=0)

        if do_classifier_free_guidance:
            # 复制无条件嵌入，并与图像嵌入合并
            uncond_image_prompt_embeds = torch.cat([uncond_image_prompt_embeds] * num_images_per_prompt, dim=0)
