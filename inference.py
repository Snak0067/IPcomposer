...
def main():
    ...
    print("load pretrained SD1.5 model")
    pipe = StableDiffusionPipeline.from_pretrained(
        args.pretrained_model_name_or_path, torch_dtype=weight_dtype
    )
    print("init IPComposer model..")
    model = IpComposerModel.from_pretrained(args)
    
    load_model(model, args)
            
    print("load pretrained ip-adapter")
    ...
    ip_adapter_image = Image.open(args.test_ip_adapter_image)
    ip_adapter_image.resize((256, 256))
    
    model.set_scale(scale=1.0)
    image_prompt_embeds = model.get_image_embeds(
        device=accelerator.device,
        pil_image=ip_adapter_image, 
    ...
    for instance_id in range(args.num_images_per_prompt):
        save_path = os.path.join(args.output_dir,f"{args.output_name}_{timestamp}.png")
        images[instance_id].save(save_path)
        print(f"generate image save as {save_path}")
